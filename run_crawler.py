...
def run_crawler(relative_path,start_url='',depth=None,no_next_crawl=None,other_options=None):
    # 检查配置环境
    ...
    if (no_next_crawl != None):
        exec_order += ' -a no_next_crawl=' + str(no_next_crawl)
...
def run_crawler_scanner(scanned_url,depth = 2, no_next_crawl = None):
    this_relative_path = '\crawler_scanner & scrapy crawl scanner'
    run_crawler(this_relative_path,start_url = scanned_url, depth = depth,no_next_crawl=no_next_crawl)

def run_crawler_scanner_splash(scanned_url,depth = 2,no_next_crawl = None):
...
if __name__ == '__main__':
    ...
    run_crawler_scanner_splash('http://owasptest.409dostastudio.pw/index.php?page=add-to-your-blog.php',depth=1, no_next_crawl = 'T')
